...
export class AsignarExamenesComponent implements OnInit {
  ...
  examenes: Examen[] = [];
  mostrarColumnasExamen: string[] = ['nombre', 'asignatura', 'eliminar'];
  mostrarColumnasExamenCurso: string[] = ['id','nombre', 'asignaturas', 'eliminar'];
  tabIndex: number = 0;

  dataSource: MatTableDataSource<Examen>;
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  pageSizeOptions: number[] = [3,5,10,20,100];
  ...
  seleccionarExamen(event: MatAutocompleteActivatedEvent): void {
    ...
    if(!this.existe(examen.id)){
      ...
      Swal.fire(
        'Error',
        `El examen ${examen.nombre} ya esta asignado en el curso`,
        'error'
      );
    }
  ...
  private existe(id: number): boolean {
    let existe =  false;
    console.log(this.curso.examenes);
  ...
  asignar(): void {
    console.log(this.eliminarDelAsignar);
    ...
    this.cursoService.asignarExamenes(this.curso, this.examenesAsignar)
      .subscribe(curso => {
        ...
        Swal.fire(
         'Asignados:',
         `Examenes asignado con exito al curso ${curso.nombre}`,
         'success'
        );
        this.tabIndex = 2;
      })
  ...
  eliminarExamenDelCurso(examen: Examen): void {
    // this.cursoService.eliminarExamen(examen)
